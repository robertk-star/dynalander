...
function platformGroup(value: string | undefined) {
...
function summarizeWithCostPerResult(rows: any[]) {
  const totals = summarizeNumbers(rows);
  return { spend: money(totals.spend), results: numberText(totals.results), costPerResult: totals.results ? money(totals.costPerResult) : '—', impressions: numberText(totals.impressions), clicks: numberText(totals.clicks), ctr: percent(totals.ctr), cpc: money(totals.cpc), cpm: money(totals.cpm) };
}

function mapInsightRows(rows: any[], nameField: string, idField: string) {
  return rows.map((row) => ({
    id: row[idField] || '—', name: row[nameField] || '—', campaignId: row.campaign_id || '—', campaignName: row.campaign_name || '—', adSetId: row.adset_id || '—', adSetName: row.adset_name || '—',
...
function mapDeviceSummary(rows: any[]) {
  ...
  rows.forEach((row) => { const group = deviceGroup(row.impression_device); grouped.set(group, [...(grouped.get(group) || []), row]); });
  return ['Mobile', 'Desktop', 'Other / Unknown'].filter((group) => grouped.has(group)).map((group) => ({ device: group, ...summarize(grouped.get(group) || []) }));
}

function mapAdDeviceRows(rows: any[]) {
  return rows.map((row) => ({
    id: `${row.ad_id || 'unknown'}-${row.impression_device || 'unknown'}`, device: deviceGroup(row.impression_device), rawDevice: row.impression_device || '—', adId: row.ad_id || '—', adName: row.ad_name || '—', adSetName: row.adset_name || '—', campaignName: row.campaign_name || '—',
    results: numberText(rowResults(row)), spend: money(row.spend), impressions: numberText(row.impressions), clicks: numberText(row.clicks), ctr: percent(row.ctr), cpc: money(row.cpc), cpm: money(row.cpm)
  }));
}

function mapPlatformSummary(rows: any[]) {
  const grouped = new Map<string, any[]>();
  rows.forEach((row) => { const group = platformGroup(row.publisher_platform); grouped.set(group, [...(grouped.get(group) || []), row]); });
  return ['Facebook', 'Instagram', 'Messenger', 'Audience Network', 'Other / Unknown'].filter((group) => grouped.has(group)).map((group) => ({ platform: group, ...summarizeWithCostPerResult(grouped.get(group) || []) }));
}

function mapPlatformRows(rows: any[]) {
  return rows.map((row) => ({
    id: `${row.publisher_platform || 'unknown'}-${row.ad_id || 'account'}`, platform: platformGroup(row.publisher_platform), rawPlatform: row.publisher_platform || '—', adId: row.ad_id || '—', adName: row.ad_name || '—', adSetName: row.adset_name || '—', campaignName: row.campaign_name || '—',
...
export async function GET(request: Request) {
  ...
  if (!env.configured || !configured) return Response.json({ ok: false, source: 'not_configured', error: 'Meta ENV is not configured.', range, summary: null, campaigns: [], adSets: [], ads: [], adDeviceSummary: [], adDeviceRows: [], platformSummary: [], platformRows: [] });
  if (selected && selected !== configured && selected !== 'act_meta-connected-account') return Response.json({ ok: false, source: 'active_account_mismatch', error: 'Selected active account does not match connected Meta account.', selected, configured, range, summary: null, campaigns: [], adSets: [], ads: [], adDeviceSummary: [], adDeviceRows: [], platformSummary: [], platformRows: [] });
  ...
  const [accountResult, campaignResult, adSetResult, adResult, adDeviceResult, platformResult] = await Promise.all([
  ...
  if (!accountResult.response.ok) return Response.json({ ok: false, source: 'meta_dashboard_summary_failed', error: accountResult.json?.error?.message || 'Could not read Meta summary.', range, summary: null, campaigns: [], adSets: [], ads: [], adDeviceSummary: [], adDeviceRows: [], platformSummary: [], platformRows: [] });

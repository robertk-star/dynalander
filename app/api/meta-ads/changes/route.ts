...
export async function GET(request: Request) {
  ...
  if (!client) {
    return Response.json({ ok: false, source: 'local', changes: [], error: 'Database is not configured.' });
  }

  const { data, error } = await client
    .from('meta_change_log')
    .select('id,campaign_id,ad_set_id,ad_id,asset_type,asset_position,old_value,new_value,change_source,detected_at,review_after_date')
  ...
  if (error) {
    return Response.json({ ok: false, source: 'database', changes: [], error: error.message }, { status: 500 });
  }
